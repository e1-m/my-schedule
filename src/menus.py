...
def edit_conference_options(conference):
    return {
        ...
        3: lambda: conference.set_new_start_time(scan_time(texts['edit_conference']['new_start_time_input'])),
        ...
        7: lambda: schedule.delete_conference(conference),
    }
...
def edit_conference_text(conference):
    return (f"1. {texts['edit_conference']['title']}{conference.title}\n"
            ...
            f"7. {texts['edit_conference']['delete']}\n"
            f"{texts['back']}")
...
def add_conference(day: str):
    ...
    if not schedule.add_conference(day, Conference(title=title, start_time=start_time, link=link)):
        print(texts['add_conference']['duration_exception'])
        os.system('pause')
    schedule.get_daily_schedule(day).sort(key=lambda conference: conference.start_time.get_in_minutes())
